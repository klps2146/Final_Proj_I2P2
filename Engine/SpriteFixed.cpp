...
// 這裡有魔改
namespace Engine {
    SpriteFixed::SpriteFixed(std::string img, float x, float y, float w, float h, float anchorX, float anchorY,
    ...
    void SpriteFixed::SetSpriteSource(float sx, float sy, float sw, float sh) {
        SourceX = sx;
        SourceY = sy;
        SourceW = sw;
        SourceH = sh;
    ...
    void SpriteFixed::SetSize(float w, float h) {
        Size.x = w;
        Size.y = h;
    }
    ...
    void SpriteFixed::Update(float deltaTime) {
